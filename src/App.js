...
export default function App() {
  ...
  useEffect(() => {
    fetchPopularSongs();
    fetchTimeCapsuleSongs();
    handleMoodSearch('happy pop');
    const storedHistory =
  ...
  const searchSongs = async (
    ...
    const query =
      type === 'track' ? searchKeyword : `${type}:"${searchKeyword}"`;
  ...
  const fetchTimeCapsuleSongs = async () => {
    ...
    const randomYear =
      Math.floor(Math.random() * (currentYear - 1980 + 1)) + 1980;
    try {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-black dark:bg-gray-900 dark:text-white">
      <main className="flex-1 p-8 mb-20">
        <header className="flex justify-between items-center mb-10">
          ...
          <button
            ...
            className="p-2 rounded-full bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 text-gray-800 dark:text-white shadow-lg transform transition-transform duration-200 hover:scale-110"
          >
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {availableGenres.map((genre) => (
                <button
                  ...
                  className="bg-gradient-to-r from-purple-400 to-pink-500 hover:from-purple-500 hover:to-pink-600 text-white font-bold py-2 px-4 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105"
                >
                  {genre}
                </button>
              ))}
        ...
        <section className="mt-10 bg-white dark:bg-gray-800 rounded-lg p-6 mb-8 shadow-lg">
          <h2 className="text-2xl font-semibold mb-6">
            Songs from a Random Year
          </h2>
          <div className="flex space-x-4 mb-5">
            <button
              onClick={fetchTimeCapsuleSongs}
              className="bg-gradient-to-r from-green-400 to-blue-500 hover:from-green-500 hover:to-blue-600 text-white font-bold py-2 px-4 rounded-full shadow-lg transform transition-transform duration-200 hover:scale-105"
            >
        ...
        <section className="mt-10 bg-white dark:bg-gray-800 rounded-lg p-6 mb-8 shadow-lg">
          <h2 className="text-2xl font-semibold mb-6">Songs for Your Mood</h2>
          <div className="flex space-x-4 mb-5">
            {moods.map((mood) => (
              <button
                key={mood.name}
      ...
      {showScrollButton && ( // Conditionally render the button
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 bg-gradient-to-r from-blue-400 to-indigo-500 hover:from-blue-500 hover:to-indigo-600 text-white p-4 rounded-full shadow-lg transition-opacity duration-300 transform transition-transform duration-200 hover:scale-110"
        >
