      ...
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
        {[...Array(10)].map((_, index) => (
          <div key={index} className="flex-none animate-pulse">
            <div className="bg-gray-700 mb-2 rounded w-full h-48"></div>
      ...
      {props.songs.map((song) => {
        ...
        return (
          ...
          >
            ...
            <p className="text-gray-400">By <span className="cursor-pointer hover:underline hover:text-[#3B82F6]" onClick={() => props.onArtistClick(song.artists[0].name)}>{song.artists[0].name}</span></p>
            <button
